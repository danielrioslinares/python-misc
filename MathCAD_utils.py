...
def SAPWin2MathCAD(sapwin_exp):
	...
	for sapwin_pol in sapwin_exp.split("-" * 78):
		...
		ss = []
		for s in [sapwin_pol_par.split(')')[1:][0] for sapwin_pol_par in sapwin_pol.split('(') if sapwin_pol_par not in ("")]:
			if s == "":
				ss.append(0)
			elif s == " s":
				ss.append(1)
		...
		for i,sapwin_pol_par in zip(ss,sapwin_pol_pars):
...
if __name__ == "__main__":
	...
	mathcad_exp = SAPWin2MathCAD(sapwin_exp)
